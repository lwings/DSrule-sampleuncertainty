...
for k,v in data_dic.items() :
    if k<3000:
        ...
        trainLabel.append(v[-2])
    else:
        testVector.append(v[0:12])
        testLabel.append(v[-2])
        testSeq.append(k)

x_data = np.array(trainVector)
y_data = np.array(trainLabel).reshape(-1,1)
...
# add hidden layer
l1 = add_layer(xs, 12, 10, activation_function=tf.nn.relu)
# add output layer
